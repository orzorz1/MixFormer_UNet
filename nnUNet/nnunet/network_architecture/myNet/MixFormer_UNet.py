...
model_patch_size = [32,128,128]   # patch_size
model_batch_size = 4              # batch_size
...
class custom_net(SN):
    ...
    def __init__(self, num_classes):
        ...
        self.model = MixFormer_UNet_3D(1, num_classes)
        ######## self.model 设置自定义网络 by Sleeep ########
        
        self.name = "MixFormer_UNet_3D"
...
class TransformerBlock(nn.Module):
    def __init__(self, in_channel, out_channel, nhead, num_layers, dim_feedforward):
        super(TransformerBlock, self).__init__()
        self.in_channel = in_channel
        self.out_channel = out_channel

        self.encoder_layers = TransformerEncoderLayer(d_model=in_channel, nhead=nhead,
                                                      dim_feedforward=dim_feedforward)
        self.transformer_encoder = TransformerEncoder(self.encoder_layers, num_layers=num_layers)

    def forward(self, x):
        n, _, h, w, d = x.shape
        x = x.permute(0, 2, 3, 4, 1).contiguous()  # Change shape to (n, h, w, d, c)
        x = x.view(n, h * w * d, self.in_channel)  # Flatten spatial dimensions

        x = self.transformer_encoder(x)

        x = x.view(n, h, w, d, self.out_channel)  # Reshape back to 3D
        x = x.permute(0, 4, 1, 2, 3).contiguous()  # Change shape back to (n, c, h, w, d)
...
class ABF(nn.Module):
    def __init__(self, in_channel, mid_channel, out_channel):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv3d(mid_channel, in_channel, kernel_size=1, bias=False),
            nn.BatchNorm3d(in_channel),
        )
        self.transformer = nn.Sequential(
            TransformerBlock(in_channel, out_channel, 4, 1, 512),
            nn.BatchNorm3d(out_channel),
        )
        self.att_conv = nn.Sequential(
            nn.Conv3d(in_channel * 2, 2, kernel_size=1),
            nn.Sigmoid(),
            )
    ...
    def forward(self, a, b):
        ...
        b = F.interpolate(b, (h, w, d), mode="nearest")
        # fusion
        z = torch.cat([a, b], dim=1)
        z = self.att_conv(z)
        a = a * z[:, 0].view(n, 1, h, w, d) + b * z[:, 1].view(n, 1, h, w, d)

        b = self.transformer(a)

        return b


class MixFormer_UNet_3D(nn.Module):
    def __init__(self, channel_in, channel_out):
        ...
        self.abf_1 = ABF(64, 128, 64)
        self.abf_2 = ABF(128, 256, 128)
        self.abf_3 = ABF(256, 512, 256)
    ...
    def forward(self, x):  # [1, 1, 64, 64, 32]
        # 1：进行编码过程
        feature_1 = self.left_conv_1(x)  # [1, 64, 64, 64, 32]
        x = self.pool_1(feature_1)

        feature_2 = self.left_conv_2(x)  # [1, 128, 32, 32, 16]
        x = self.pool_2(feature_2)

        feature_3 = self.left_conv_3(x)  # [1, 256, 16, 16, 8]
        x = self.pool_3(feature_3)

        feature_4 = self.left_conv_4(x)  # [1, 512, 8, 8, 4]
        x = self.pool_4(feature_4)

        feature_3 = self.abf_3(feature_3, feature_4)
        feature_2 = self.abf_2(feature_2, feature_3)
        feature_1 = self.abf_1(feature_1, feature_2)
